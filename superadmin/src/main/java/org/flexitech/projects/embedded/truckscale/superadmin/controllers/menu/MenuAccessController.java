package org.flexitech.projects.embedded.truckscale.superadmin.controllers.menu;
...
public class MenuAccessController {
	...
	public String setupRoleAccess(Model model, HttpServletRequest httpRequest) {
		...
		SuperAdminUserDTO sessionUser = (SuperAdminUserDTO) httpRequest.getSession()
				.getAttribute(CommonConstants.SESSION_LOGGED_USER);

		List<MenuAccessTreeDTO> accessTreeList =
		menuRoleAccessService.getSelectedAccessTree(sessionUser.getUserRoleId());
	...
	public @ResponseBody String getTreeJson(Model model, @RequestParam(value = "roleId", required = false) Long roleId,
		...
		SuperAdminUserDTO loginUser = (SuperAdminUserDTO) request.getSession().getAttribute(CommonConstants.SESSION_LOGGED_USER);
	...
	public String saveRoleAccessRight(@ModelAttribute("accessTreeDtoList") MenuAccessListDTO accessTreeDtoList,
		...
		try {
			...
			menuRoleAccessService.saveMenuAccessForSupAdmin(accessTreeDtoList, loginUser);
