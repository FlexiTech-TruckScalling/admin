...
<div class="">
	<div class="card shadow mb-4">
		<div class="card-header py-3">
			<h6 class="m-0 font-weight-bold text-primary">${customerDTO.name }
				Vehicle Manage</h6>
		</div>
		<div class="card-body">

			<form:form action="customer-vehicle-manage.fxt"
				modelAttribute="customerVehicleDTO" method="post" id="common-form">
				<form:hidden path="id" />
				<div class="row g-3">
					<!-- Prefix -->
					<div class="col-md-3">
						...
						<form:input path="prefix" type="text"
							class="form-control required" placeholder="Vehicle Prefix" />
						<span class="text-danger input-error-msg"
							data-label="Vehicle Prefix "></span>
					</div>
					...
					<div class="col-md-3">
						...
						<form:input path="number" type="text"
							class="form-control required" placeholder="Vehicle Number" />
						<span class="text-danger input-error-msg"
							data-label="Vehicle Number "></span>
					</div>
					...
					<div class="col text-left">
						...
						<c:if test="${not empty customerVehicleDTO.id }">
							<button type="button" data-action="customer-vehicle-delete.fxt"
								data-id="${customerVehicleDTO.id }"
								class="btn btn-danger delete-data">Delete</button>
						</c:if>
						<a href='<c:url value="/customer-manage.fxt?id=${customerId }"/>'
				...
				<table class="table table-striped">
					...
					<tbody>
						...
						<c:if test="${not empty customerVehicleList }">
							<c:forEach items="${customerVehicleList }" var="counter"
								varStatus="loop">
								<tr
